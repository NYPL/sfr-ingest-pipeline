...
class DataObject(object):
    ...
    def addClassItem(self, listAttrib, classType, **itemDict):
        if listAttrib not in dir(self):
            raise DataError('Field {} not valid for {}'.format(
                listAttrib,
                self.__class__.__name__
            ))
        self[listAttrib].append(classType.createFromDict(**itemDict))
...
class WorkRecord(DataObject):
    ...
    def __dir__(self):
        return [
            'identifiers', 'instances', 'subjects', 'agents', 'links', 'measurements',
            'dates', 'uuid', 'language', 'title', 'sub_title', 'alt_titles',
            'sort_title', 'medium', 'summary', 'series', 'series_position',
            'primary_identifier', 'rights'
        ]
