...
class V3Search {
  ...
  async loadWorks(works, recordType) {
    ...
    for (let i = 0; i < works.length; i += 1) {
      ...
      // dbWork.identifiers = await this.getIdentifiers('work', dbWork.id)
      dbWork.instances = null
      dbWork.editions = null
      dbWork.edition_count = 0
      dbWork.edition_range = works[i].edition_range
      this.getInnerRecords(works[i], dbWork, recordType)
  ...
  getEditions(editionIds) {
    return this.dbConn.loadEditions(editionIds, 3)
  }

  getInstances(instanceIds) {
    return this.dbConn.loadInstances(instanceIds, 3)
  }
